...
- При `/start` в консоль выводится **зелёная строка** с форматом `user / bot / start`.
- При активном webhook бот пытается автоматически удалить webhook и продолжить polling.
- Ошибки токенов (например 401) логируются без длинного traceback, проблемный бот пропускается.
...
```text
user=Ivan Ivanov(@ivan|id=123) / bot=@my_bot / start / chat=123
```
